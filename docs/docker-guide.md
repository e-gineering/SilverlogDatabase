...
| `environment` / `env_file` | Postgres's official image reads `POSTGRES_USER`, `POSTGRES_PASSWORD`, `POSTGRES_DB` on first startup to create the superuser role and initial database. We load these from `.env` instead of hardcoding them. |
| `ports: ["5432:5432"]` | `host:container`. Maps a port on your machine to Postgres's port inside the container, so tools on your host (`psql`, your .NET app, DBeaver/TablePlus) can reach it at `localhost:<port>`. |
